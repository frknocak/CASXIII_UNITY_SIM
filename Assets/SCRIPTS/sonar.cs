...
public class sonar : MonoBehaviour
{
    ...
    //List<float> minValues = new List<float> { 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51, 51 };
    //  list.Index ilk teması 1. rayden almazsa index sırasında atlama olduğu için içine veri alamıyor. Bu yüzden içine index niyetiyle sayı girdim
    //  deafult olarak maks değerden yüksek bir değer verdim daha büyük veri gelemesin diye
    RaycastHit hit;
    ...
    private void Start()
    {
        CheckForFile(); // masaüstünde gerekli dosya mevcut mu diye bakan kod. Yoksa Oluşturuyor.
    ...
    private void FixedUpdate()
    {
        ...
        for (a = 0; a < 25; a++)
        {
            ...
            if (Physics.Raycast(raycasts[a], out hit, 50))
            {
                ...
                int hitDistanceInt = Mathf.CeilToInt(hit.distance);

                placeHolderLists[a][intFacingAngle] = hitDistanceInt;   //a. listenin b. elemanını hit.distance ile değiştirmek
            }
            else
            {
                //print("engel yok");
            ...
            if (rotation % 360f == 0)
            {
                ...
                //TransposeData();
                WriteArraysToTextFile(raycastPerDegreeList);
