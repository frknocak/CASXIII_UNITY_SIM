...
public class UDP_port : MonoBehaviour
{
    ...
    // yorum satırları gerçekçi hareket algısı üzerine hazırlandığı için UDP kontrolde PID gerektiriyor.
    private void Start()
    {
        translationAmount = new Vector3(0.0f, 0.015f, 0.0f); //Her bir veride ne kadar yukarı çıkacak
        int udpPort = 12345;
    ...
    // kod buğranın bu fonksiyonu neden yazdı hiçbir fikrim yok.
    /*    public static int[] ConvertByteArrayToIntArray(byte[] byteArray)
        {
            int[] intArray = new int[byteArray.Length / 4]; // 4 byte'lık integer değerler olduğunu varsayıyoruz.
    ...
    private void HandleMessage(byte[] ints)
    {
        ...
        if (r == 1)
        {
            ...
            if (Mathf.Abs(ROV.transform.position.y - simDepth) > tolerance)
            {
                ...
                {
                    translationAmount = new Vector3(0.0f, (-1 * z) / 3900, 0.0f);
                    ROV.transform.Translate(translationAmount);
                }
